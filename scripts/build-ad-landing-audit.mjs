...
const CONCEPT_GROUPS = {
  outcomes: {
    leads:
      /\b(?:lead|leads|prospect\w*|contactos? cualificad\w*|oportunidad(?:es)? comercial\w*)\b/i,
    appointments:
      /\b(?:citas?|reuniones?|agenda(?:r|mos|da)?|appointment\w*|demo(?:stracion)?)\b/i,
    clients: /\b(?:clientes?|pacientes?|compradores?|alumnos?|matricul\w*)\b/i,
    sales:
      /\b(?:ventas?|factur\w*|ingresos?|cerrar|conversion(?:es)?|rentabilidad|roi)\b/i,
    growth:
      /\b(?:crecer|crecimiento|escalar|multiplic\w*|resultados?|pipeline)\b/i,
    savings:
      /\b(?:ahorr\w*|reduc\w* costes?|menos coste|cpl|coste por lead)\b/i,
    visibility:
      /\b(?:visibilidad|posicionamiento|trafico|alcance|reputacion)\b/i,
  },
  audiences: {
    b2b: /\b(?:b2b|empresas?|negocios?|pymes?|equipos? comercial\w*|directivos?|ceos?)\b/i,
    health:
      /\b(?:clinicas?|dentales?|dentistas?|medicos?|sanitari\w*|pacientes?|fisioterap\w*|salud)\b/i,
    realEstate:
      /\b(?:inmobiliari\w*|agentes? inmobiliari\w*|promotor\w*|propietarios?|viviendas?)\b/i,
    legal: /\b(?:abogados?|despachos?|juridic\w*|legal|concursal\w*)\b/i,
    homeServices:
      /\b(?:reformas?|construct\w*|fontaner\w*|cerrajer\w*|hogar|obras?)\b/i,
    solar: /\b(?:solar(?:es)?|fotovoltaic\w*|energia|placas?)\b/i,
    automotive:
      /\b(?:coches?|vehiculos?|automocion|talleres?|concesionari\w*)\b/i,
    hospitality: /\b(?:hoteles?|restaurantes?|hosteleri\w*|turismo)\b/i,
    ecommerce: /\b(?:ecommerce|e-commerce|tiendas? online|shopify)\b/i,
    education:
      /\b(?:coaches?|formadores?|academias?|escuelas?|cursos?|alumnos?)\b/i,
    local: /\b(?:negocios? locales?|comercios?|profesionales? locales?)\b/i,
  },
  mechanisms: {
    paidMedia:
      /\b(?:meta ads|facebook ads|instagram ads|google ads|publicidad|campanas? de pago|paid media)\b/i,
    outbound:
      /\b(?:outbound|prospeccion|cold email|email frio|linkedin|llamadas? en frio|sdr)\b/i,
    automation:
      /\b(?:automatiz\w*|inteligencia artificial|\bia\b|crm|workflow|agentes? de voz)\b/i,
    seo: /\b(?:seo|posicionamiento organico|buscadores?|google maps)\b/i,
    funnel: /\b(?:embudo|funnel|landing|vsl|webinar|lead magnet)\b/i,
    qualification:
      /\b(?:cualific\w*|filtr\w*|valid\w*|perfil ideal|scoring)\b/i,
    ...
    consulting:
      /\b(?:consultoria|auditoria|diagnostico|estrategia|mentoria)\b/i,
    training: /\b(?:formacion|curso|programa|masterclass|taller|comunidad)\b/i,
  },
};

const CONCEPT_LABELS = {
  leads: "leads o contactos cualificados",
  appointments: "citas o reuniones",
  clients: "clientes o pacientes",
  sales: "ventas o facturación",
  growth: "crecimiento",
  savings: "ahorro o reducción de coste",
  visibility: "visibilidad",
  b2b: "empresas y equipos B2B",
  health: "clínicas y salud",
  realEstate: "inmobiliario",
  legal: "servicios legales",
  homeServices: "reformas y servicios del hogar",
  solar: "solar y energía",
  automotive: "automoción",
  hospitality: "hostelería y turismo",
  ecommerce: "ecommerce",
  education: "formación",
  local: "negocios locales",
  paidMedia: "publicidad de pago",
  outbound: "prospección outbound",
  automation: "automatización o IA",
  seo: "SEO",
  funnel: "funnel o landing",
  qualification: "cualificación",
  nurturing: "seguimiento",
  marketplace: "marketplace o comparador",
  consulting: "consultoría o diagnóstico",
  training: "formación o masterclass",
};

const conceptLabels = (ids) =>
  unique(ids.map((id) => CONCEPT_LABELS[id] || id));
...
const VERTICAL_RULES = [
  [
    "clinicas-salud",
    /clinica|salud|dental|dentist|medic|pacient|fisioterap|veterin|audifon/i,
  ],
  [
    "reformas-hogar",
    /reforma|hogar|obra|constru|fontaner|cerrajer|piscina|tejado/i,
  ],
  ...
  [
    ...
    /marketplace|directorio|presupuesto|comparador|profesional/i,
  ],
...
function normalUrl(value) {
  try {
    ...
    for (const key of url.searchParams.keys()) {
      if (
        /^(?:utm_.+|fbclid|gclid|campaign|campaign_id|ad_id|hsa_.+)$/i.test(key)
      ) {
        removable.push(key);
      }
    }
...
function isPoorDestination(value) {
  ...
  return (
    SOCIAL_DESTINATION.test(parts.host) ||
    IRRELEVANT_DESTINATION.test(parts.path)
  );
}
...
function isAuditReadyAd(ad) {
  ...
  const withoutViewer = normalized
    .replace(
      /la informacion sobre este anuncio puede variar segun la ubicacion/g,
      "",
    )
...
function rankEvidenceAds(ads) {
  return [...ads].sort((left, right) => {
    ...
    return (
      ...
      clean(left.corpusKey).localeCompare(clean(right.corpusKey))
    );
  });
}
...
function compactLandingRead(capture, analysis) {
  ...
  const first = (key) => {
    ...
    if (Array.isArray(preferred))
      return preferred.length ? preferred : fallback[key] || [];
...
function choosePrimaryUrl({ ads, analysis, capture, company }) {
    ...
    ...(capture?.pages || [])
      .filter(
        (page) =>
          ...
          ["landing", "conversion"].includes(page.role),
      )
      .map(pageUrl),
  ].filter(Boolean);
  ...
  const frequent = [...frequency.entries()].sort(
...
function chooseCapturedPage(capture, primaryUrl) {
  const pages = (capture?.pages || []).filter(
    (page) =>
      page.status === "captured" &&
      (page.image?.file || page.text?.h1 || page.text?.headings?.length),
  );
  ...
  const ranked = pages
    ...
    .sort(
      (a, b) =>
        b.score - a.score || clean(a.page.id).localeCompare(clean(b.page.id)),
    );
...
function landingEvidence(page, read, primaryUrl, capture) {
  return {
    ...
    excerpt:
      excerpt(page?.text?.excerpt || read.promise || read.offer, 420) || null,
  };
}

function dimensionBase(id, status, score, rationale, signals = {}) {
  const definition = [...QUALITY_DIMENSIONS, CAPTURE_DIMENSION].find(
    (item) => item.id === id,
  );
  return {
    id,
...
function semanticDimension({ id, adText, landingText, group, missingMessage }) {
  ...
  const missing = adConcepts.filter((concept) => !shared.includes(concept));
  const landingOnly = landingConcepts.filter(
    (concept) => !shared.includes(concept),
  );
  ...
  if (!observed(landingText)) {
    return dimensionBase(
      ...
      "not_observed",
      null,
      `${missingMessage} El anuncio sí activa ${naturalList(conceptLabels(adConcepts))}.`,
      signals,
    );
  }
  if (shared.length >= 2 || (shared.length === 1 && adConcepts.length === 1)) {
    return dimensionBase(
      ...
      `El anuncio y la landing mantienen ${naturalList(conceptLabels(shared))}.`,
      signals,
    );
  }
  if (shared.length === 1 || lexical >= 0.12) {
    return dimensionBase(
      ...
      shared.length
        ? `La landing conserva ${naturalList(conceptLabels(shared))}, pero deja fuera ${naturalList(conceptLabels(missing)) || "parte del foco publicitario"}.`
        : `La redacción conserva parte del vocabulario, pero no permite confirmar ${naturalList(conceptLabels(adConcepts))}.`,
      signals,
    );
  }
  if (adConcepts.length && landingConcepts.length) {
    return dimensionBase(
      ...
      "leak",
      28,
      `El anuncio enfatiza ${naturalList(conceptLabels(adConcepts))}; la landing pasa a ${naturalList(conceptLabels(landingOnly)) || "otro foco sin equivalencia explícita"}.`,
      signals,
    );
  }
  return dimensionBase(
    ...
    "partial",
    52,
    `La landing contiene información, pero no permite confirmar ${naturalList(conceptLabels(adConcepts))} con suficiente claridad.`,
    signals,
  );
}
...
function classifyCta(value) {
  const text = normalizeText(value);
  if (!text) return null;
  if (/\b(?:whatsapp|mensaje|mensajes|escribe|escribenos)\b/.test(text))
    return "message";
  if (/reserv|agenda|cita|reunion|llamad|diagnost|auditoria|demo/.test(text))
    return "booking";
  if (/presupuesto|cotiza|valoracion|propuesta/.test(text)) return "quote";
  if (/registr|apunt|inscri|plaza|acced|ver video|webinar/.test(text))
    return "register";
  if (/descarg|guia|ebook|informe/.test(text)) return "download";
  if (/compr|empieza|comenz|contrata|solicita|quiero/.test(text))
    return "start";
  if (/contact|habla|consulta/.test(text)) return "contact";
  if (
    /mas informacion|learn more|ver detalles|descubre|saber mas|see details/.test(
      text,
    )
  )
    return "learn";
...
function chooseLandingCta(read, capturedPage) {
  if (classifyCta(read.primaryCta) && classifyCta(read.primaryCta) !== "other")
    return read.primaryCta;
  return (
    (capturedPage?.text?.ctas || []).find((cta) => {
      const text = normalizeText(cta);
      return (
        classifyCta(cta) !== "other" &&
        !/(?:privacidad|privacy|cookie|legal|terms)/.test(text)
      );
    }) ||
    read.primaryCta ||
    null
...
function ctaDimension(adCtas, landingCta, captureAvailable) {
  ...
  const signals = {
    adFamilies,
    landingFamily,
    adCtas: unique(adCtas).slice(0, 6),
    landingCta: clean(landingCta) || null,
  };
  if (!adFamilies.length) {
    return dimensionBase(
      ...
      "not_observed",
      null,
      "No se observó un CTA explícito en los anuncios recuperados.",
      signals,
    );
  }
  if (!landingFamily) {
  ...
  if (adFamilies.includes(landingFamily)) {
    return dimensionBase(
      ...
      "aligned",
      94,
      "La acción anunciada continúa con el mismo tipo de siguiente paso.",
      signals,
    );
  }
  if (
    adFamilies.includes("learn") ||
    (adFamilies.includes("contact") &&
      ["booking", "quote", "message"].includes(landingFamily)) ||
    (adFamilies.includes("start") &&
      ["booking", "contact", "quote"].includes(landingFamily))
  ) {
    return dimensionBase(
      "cta",
      "partial",
...
function hasProof(value) {
  const text = normalizeText(value);
  return /(?:casos? de exito|testimoni|resenas?|opiniones?|clientes? confian|\b\d+[+%]?\b|anos? de experiencia|proyectos? publicados?)/.test(
    text,
  );
}
...
function proofDimension(adText, landingProof, landingText, captureAvailable) {
  ...
  const signals = {
    adClaimsProof: adHasProof,
    landingShowsProof: landingHasProof,
  };
  if (landingHasProof) {
    return dimensionBase(
      "proof",
...
function commercialSignals(value) {
  ...
  return unique([
    ...
    /garantizamos|garantizado|garantizada|bajo garantia|garantia total|si no .* no paga|no pagas|devolu/.test(
      text,
    )
      ? "guarantee"
      : null,
    /(?:\d+[.,]?\d*)\s*(?:€|eur|euros?|\/mes)/.test(text) ? "price" : null,
    /pago por (?:lead|cita|resultado)|a exito|por resultados/.test(text)
      ? "performancePricing"
      : null,
    /sin permanencia|cancela|contrato|minimo .* meses?/.test(text)
      ? "contract"
      : null,
  ]);
}

function commercialTermsDimension(
  adText,
  read,
  landingPageText,
  captureAvailable,
  ...
  const landingText = [read.price, read.guarantee, landingPageText]
    .filter(observed)
    .join(" · ");
  ...
  if (shared.length === adSignals.length) {
    return dimensionBase(
      ...
      "aligned",
      92,
      "Las condiciones comerciales observadas mantienen continuidad.",
      signals,
    );
  }
  if (shared.length) {
    return dimensionBase(
      ...
      "partial",
      62,
      "Solo una parte de las condiciones comerciales se conserva.",
      signals,
    );
  }
  return dimensionBase(
    ...
    "leak",
    30,
    "Las condiciones explícitas cambian entre anuncio y landing.",
    signals,
  );
}

function destinationDimension(ads, primaryUrl, capturedPage, capture) {
  const destinations = ads
    .map((ad) => normalUrl(ad.landingUrl))
    .filter(Boolean);
  ...
  const hostMatchCount = primary
    ? destinations.filter((url) => safeUrlParts(url)?.host === primary.host)
        .length
    : 0;
  const signals = {
    observedDestinations: uniqueDestinations.length,
    poorDestinations: unique(destinations.filter(isPoorDestination)).slice(
      0,
      8,
    ),
    primaryHostShare: destinations.length
      ? Number((hostMatchCount / destinations.length).toFixed(3))
      : null,
    primaryUrl: primaryUrl || null,
    capturedUrl: pageUrl(capturedPage),
    redirectObserved: Boolean(
      primary && captured && primary.host !== captured.host,
    ),
  };
  ...
  if (
    primary &&
    captured &&
    primary.host === captured.host &&
    primary.path === captured.path
  ) {
    return dimensionBase(
      "destinationFriction",
      "aligned",
      96,
      "El destino anunciado coincide con la página capturada.",
      signals,
    );
  }
...
function captureDimension(capture, capturedPage) {
  ...
  const score = planned
    ? Math.round((captured / planned) * 100)
    : capturedPage
      ? 100
      : 0;
  const status =
    score === 100 ? "aligned" : score > 0 ? "partial" : "not_observed";
  return dimensionBase(
    "captureCoverage",
    status,
    score,
        ...
        : "No existe una captura utilizable; este dato reduce confianza, no la nota de calidad.",
    {
      ...
      blocked,
      failed,
      fullPage: Boolean(capturedPage?.fullPage),
    },
  );
}
...
function inferVertical(company, ads, landingIntelligence) {
  ...
  const explicit = ads
    .map((ad) => clean(ad.vertical))
    .find((value) => knownIds.has(value));
  ...
  const text = normalizeText(
    [
      ...
      company?.niche,
      company?.offer,
      ...ads.map((ad) => `${ad.titular || ""} ${ad.texto || ""}`),
    ]
      .join(" ")
      .slice(0, 40000),
  );
  return (
    VERTICAL_RULES.find(([, pattern]) => pattern.test(text))?.[0] ||
    "generalista"
  );
}
...
function weightedScore(dimensions) {
  const evaluated = dimensions.filter(
    (dimension) => dimension.weight > 0 && Number.isFinite(dimension.score),
  );
...
function evidenceConfidence({
  ...
  const captureCoverage =
    Number(
      dimensions.find((dimension) => dimension.id === "captureCoverage")
        ?.score || 0,
    ) / 100;
  ...
  const score = Math.round(
    100 *
      (dimensionCoverage * 0.48 +
        captureCoverage * 0.22 +
        adDepth * 0.16 +
        sourceQuality * 0.14),
  );
  const label =
    score >= 75 && capture?.coverage?.captured > 0
      ? "high"
      : score >= 45
        ? "medium"
        : "low";
...
const ACTIONS = {
  promise:
    "Repetir en el hero la promesa concreta del anuncio y explicar inmediatamente cómo se entrega.",
  audience:
    "Nombrar en el primer bloque al mismo público o caso de uso que activa el anuncio.",
  offerMechanism:
    "Conservar oferta y mecanismo entre anuncio y landing; mostrar el proceso en pasos verificables.",
  cta: "Alinear la acción prometida con el CTA principal y explicar cualquier paso intermedio.",
  proof:
    "Colocar junto a la promesa evidencia identificable y sus límites; evitar claims sin respaldo localizable.",
  commercialTerms:
    "Repetir y aclarar precio, gratuidad o garantía, incluyendo condiciones visibles antes del formulario.",
  destinationFriction:
    "Enviar el anuncio a una página comercial específica y eliminar saltos a rutas legales, sociales o genéricas.",
};

const CTA_FAMILY_LABELS = {
  message: "mensaje o WhatsApp",
  booking: "reserva de cita o reunión",
  quote: "solicitud de presupuesto",
  register: "registro",
  download: "descarga",
  start: "inicio o contratación",
  contact: "contacto",
  learn: "ampliación de información",
  other: "otra acción",
};
const COMMERCIAL_SIGNAL_LABELS = {
  free: "gratuidad",
  guarantee: "garantía",
  price: "precio",
  performancePricing: "pago por resultado",
  contract: "permanencia o contrato",
};

function actionForDimension(dimension) {
  const signals = dimension.signals || {};
  if (["promise", "audience", "offerMechanism"].includes(dimension.id)) {
    const adConcepts = Array.isArray(signals.adConcepts)
      ? signals.adConcepts
      : [];
    const shared = new Set(
      Array.isArray(signals.sharedConcepts) ? signals.sharedConcepts : [],
    );
    const missing = conceptLabels(
      adConcepts.filter((concept) => !shared.has(concept)),
    );
    const focus = naturalList(
      missing.length ? missing : conceptLabels(adConcepts),
    );
    if (dimension.id === "promise" && focus) {
      return `Haz visible “${focus}” en el hero y explica debajo el mecanismo y el límite de esa promesa.`;
    }
    if (dimension.id === "audience" && focus) {
      return `Nombra “${focus}” en el primer bloque y demuestra la especialización con un caso o prueba del mismo segmento.`;
    }
    if (dimension.id === "offerMechanism" && focus) {
      return `Explica “${focus}” con pasos, entregables y condiciones antes del formulario.`;
    }
  }
  if (dimension.id === "cta") {
    const advertised = naturalList(
      (Array.isArray(signals.adFamilies) ? signals.adFamilies : []).map(
        (family) => CTA_FAMILY_LABELS[family] || family,
      ),
    );
    const landing =
      CTA_FAMILY_LABELS[signals.landingFamily] || signals.landingFamily;
    if (advertised && landing) {
      return `Aclara la transición de “${advertised}” en el anuncio a “${landing}” en la landing, o usa el mismo siguiente paso en ambos.`;
    }
  }
  if (dimension.id === "proof") {
    return signals.adClaimsProof
      ? "Sitúa junto al hero la cifra, caso o autoridad usada en el anuncio, con fuente, alcance y límites visibles."
      : "Añade una prueba identificable junto a la promesa principal antes de pedir la acción.";
  }
  if (dimension.id === "commercialTerms") {
    const adSignals = Array.isArray(signals.adSignals) ? signals.adSignals : [];
    const shared = new Set(
      Array.isArray(signals.sharedSignals) ? signals.sharedSignals : [],
    );
    const missing = adSignals
      .filter((signal) => !shared.has(signal))
      .map((signal) => COMMERCIAL_SIGNAL_LABELS[signal] || signal);
    if (missing.length) {
      return `Repite y aclara ${naturalList(missing)} antes del formulario, incluyendo condiciones y exclusiones.`;
    }
  }
  if (dimension.id === "destinationFriction") {
    const poor = Array.isArray(signals.poorDestinations)
      ? signals.poorDestinations.length
      : 0;
    return poor
      ? `Sustituye ${poor} ${poor === 1 ? "destino secundario" : "destinos secundarios"} por la página comercial específica y conserva una única ruta medible.`
      : ACTIONS.destinationFriction;
  }
  return (
    ACTIONS[dimension.id] ||
    "Revisar esta dimensión con evidencia adicional antes de publicar cambios."
  );
}
...
function buildLeaks(dimensions) {
  return dimensions
    ...
    .sort(
      (left, right) => right.weight - left.weight || left.score - right.score,
    )
    .map((dimension, index) => ({
      dimension: dimension.id,
      label: dimension.label,
      severity:
        dimension.weight >= 18 || dimension.score <= 24
          ? "high"
          : dimension.weight >= 10
            ? "medium"
            : "low",
...
function buildActions(dimensions, qualityScore, confidence, state) {
  if (
    qualityScore === null ||
    confidence.label === "low" ||
    state === "coherent_sample"
  ) {
    return [];
  }
  return dimensions
    .filter((dimension) => {
      if (
        ...
        return false;
      const weightedLoss = (dimension.weight * (100 - dimension.score)) / 100;
      return weightedLoss >= 4;
    })
    .sort((left, right) => {
...
function auditState(score, confidence, leaks, dimensions) {
  ...
  if (leaks.some((leak) => leak.severity === "high") || score < 50)
    return "critical_leaks";
  const partials = dimensions.filter(
    (dimension) => dimension.status === "partial" && dimension.weight > 0,
  ).length;
...
function buildItem({
  ...
  const landingReadable =
    captureAvailable ||
    (Number(analysis?.capturedPages || 0) > 0 &&
      !["failed", "no_url"].includes(clean(analysis?.status)));
  ...
  const dimensions = [
    semanticDimension({
      ...
      landingText: landingReadable
        ? [read.headline, read.promise].filter(observed).join(" · ")
        : "",
      group: "outcomes",
    ...
    proofDimension(
      adText,
      landingReadable ? read.proof : null,
      landingReadable ? landingText : "",
      landingReadable,
    ),
    commercialTermsDimension(
      adText,
      landingReadable ? read : {},
      landingReadable ? landingText : "",
      landingReadable,
    ),
  ...
  const state = auditState(quality.score, confidence, leaks, dimensions);
  const actions = buildActions(dimensions, quality.score, confidence, state);
  const vertical = inferVertical(company, auditReadyAds, landingIntelligence);
  const destinationUrls = unique(
    ads.map((ad) => normalUrl(ad.landingUrl)).filter(Boolean),
  );

  return {
    id: `${id}::${Buffer.from(primaryUrl || "without-landing")
      .toString("base64url")
      .slice(0, 16)}`,
    companyId: id,
    companyName: clean(
      company?.name || analysis?.name || capture?.name || ads[0]?.name || id,
    ),
    country:
      clean(
        company?.primaryCountry ||
          company?.country ||
          capture?.primaryCountry ||
          ads[0]?.country,
      ) || "No identificado",
    vertical,
    verticalLabel:
      landingIntelligence.verticals?.[vertical]?.label ||
      "Generalista / multisegmento",
    landing: {
      url: primaryUrl || null,
...
function aggregate(items, dimensionDefinitions) {
  ...
  for (const item of items) {
    ...
    confidenceCounts[item.confidence.label] =
      (confidenceCounts[item.confidence.label] || 0) + 1;
    verticalCounts[item.vertical] = (verticalCounts[item.vertical] || 0) + 1;
  }
  const dimensions = Object.fromEntries(
    dimensionDefinitions.map((definition) => {
      const values = items.map((item) =>
        item.dimensions.find((dimension) => dimension.id === definition.id),
      );
      const evaluable = values.filter(
        (value) =>
          value?.status !== "not_observed" && Number.isFinite(value?.score),
      );
      const statuses = {};
      ...
      return [
        definition.id,
        {
          ...
          averageScore: evaluable.length
            ? Math.round(
                evaluable.reduce((sum, value) => sum + value.score, 0) /
                  evaluable.length,
              )
            : null,
          evaluable: evaluable.length,
          notObserved: values.filter(
  ...
  return {
    ...
    medianQualityScore: scored.length
      ? [...scored].map((item) => item.qualityScore).sort((a, b) => a - b)[
          Math.floor(scored.length / 2)
        ]
      : null,
    averageConfidence: items.length
      ? Math.round(
          items.reduce((sum, item) => sum + item.confidence.score, 0) /
            items.length,
        )
      : null,
    totalAds: items.reduce((sum, item) => sum + item.ads.total, 0),
    uniqueCopies: items.reduce((sum, item) => sum + item.ads.uniqueCopies, 0),
    withCapturedLanding: items.filter(
      (item) => item.landing.capture.captureStatus === "captured",
    ).length,
    stateCounts,
    confidenceCounts,
    verticalCounts,
...
export function buildAdLandingAudit({
  root = PROJECT_ROOT,
  write = true,
} = {}) {
  ...
  const analysisById = new Map(
    landingAnalysis.items.map((item) => [item.id, item]),
  );
  const captureIndexById = new Map(
    captureIndex.records.map((item) => [item.id, item]),
  );
  ...
  for (const id of ids) {
    const capturePath = path.join(
      root,
      "public/data/site-captures",
      `${id}.json`,
    );
    if (fs.existsSync(capturePath))
      captures.set(id, JSON.parse(fs.readFileSync(capturePath, "utf8")));
  }
  ...
  const result = {
    ...
    methodology: {
      ...
      statusSemantics: {
        ...
        not_observed:
          "No hay evidencia suficiente; no se interpreta como malo.",
      },
      warning:
    ...
    benchmarks: {
      universal: {
        medianFunnelSteps:
          landingIntelligence.universal?.medianFunnelSteps ?? null,
        medianFormFields:
          landingIntelligence.universal?.medianFormFields ?? null,
        fieldPresence: landingIntelligence.universal?.fieldPresence ?? {},
      },
      verticals: Object.fromEntries(
        Object.entries(landingIntelligence.verticals || {}).map(
          ([id, vertical]) => [
            id,
            {
