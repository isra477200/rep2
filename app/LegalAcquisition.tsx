...
export default function LegalAcquisition({onOpenCompany}: {onOpenCompany: (id: string) => void}) {
 ...
 return <div className={styles.shell}>
  ...
  <nav className={styles.tabs} aria-label="Herramientas para captar abogados">{([['ruta','Ruta de trabajo'],['biblioteca',`Biblioteca · ${scripts.length} textos`],['evidencia','Qué tomamos del panel'],['numeros','Rentabilidad del piloto']] as [Page,string][]).map(([id,label])=><button key={id} onClick={()=>setPage(id)} aria-current={page===id?'page':undefined}>{label}</button>)}<a href="/abogados/anuncios.html">Anuncios descargables ↗</a><a href="/abogados/medicion.html">GTM y Analytics ↗</a></nav>
  <section className={styles.landingLinks} aria-label="Centro de lanzamiento"><div><span className={styles.tag}>PÁGINAS · ANUNCIOS · CONTACTO · MEDICIÓN</span><h2>Del plan al lanzamiento.</h2><p>Landing comercial, tres modelos jurídicos, 30 imágenes + 3 vídeos y medición preparada. Contacto de RedVitalia: 919 935 237.</p></div><div><a href="/abogados/index.html">Abrir el centro de lanzamiento ↗</a><a href="/abogados/RedVitalia-abogados-kit.zip" download>Descargar todo el kit ↓</a></div></section>
  {page==='ruta' && <div className={styles.workspace}><aside className={styles.steps}><div className={styles.progress}><b>{complete.length}/12 pasos preparados</b><progress max={12} value={complete.length}/><small>Marcar un paso indica preparación, no clientes conseguidos.</small></div><nav aria-label="Pasos de captación">{chapters.map(c=><button key={c.id} onClick={()=>openChapter(c.id)} aria-current={active===c.id?'step':undefined}><span>{c.label}</span>{complete.includes(c.id)&&<b aria-label="Preparado">✓</b>}</button>)}</nav><button className={styles.downloadCsv} onClick={()=>download('RedVitalia-prospeccion-despachos.csv',prospectCsv,'text/csv;charset=utf-8')}>Descargar plantilla de contactos ↓</button></aside>
   <main ref={chapterRef} className={styles.content} aria-label={chapter.title}><div className={styles.chapterHeader}><span className={styles.eyebrow}>{chapter.label}</span><h2>{chapter.title}</h2><p>{chapter.intro}</p></div>
    ...
    {chapter.id==='creativos'&&<div className={styles.creatives}>{[{file:'creativo-seguimiento.png',title:'A · Abogados, más allá del boca a boca',alt:'Sistema RedVitalia desde 400 euros al mes: teléfono amarillo y mensaje para abogados'},{file:'creativo-recorrido.png',title:'B · Abogados, más visibilidad',alt:'Sistema RedVitalia desde 400 euros al mes: retrato ilustrativo y mensaje para abogados'}].map(c=><figure key={c.file}><a href={`/abogados/${c.file}`} target="_blank" rel="noreferrer"><img src={`/abogados/${c.file}`} alt={c.alt} loading="lazy"/></a><figcaption><b>{c.title}</b><span>Imagen generada · ilustrativa</span><a href={`/abogados/${c.file}`} download>Descargar PNG ↓</a></figcaption></figure>)}</div>}
    <div className={styles.scriptList}>{chapter.id==='creativos'&&<p className={styles.note}><a href="/abogados/anuncios.html">Abrir el estudio completo: 30 PNG + 3 vídeos, 100 textos de texto y enlaces de campaña ↗</a></p>}{scripts.filter(s=>s.chapter===chapter.id).map(item=><CopyCard key={item.id} item={item} context={context}/>)}</div>
