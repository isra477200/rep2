(() => {
  ...
  const knownPages=['index','redvitalia','segunda-oportunidad','herencias','divorcios','manual','anuncios','medicion','privacidad','gracias'];
  ...
  const activeGTM=/^GTM-[A-Z0-9]{4,}$/.test(cfg.gtmId||'');
  const active=activeGTM||/^G-[A-Z0-9]{4,}$/.test(cfg.ga4Id||'');
  const api={pageId,pageKind,campaign,consent:choice===true,hasResponded:choice!==null,track};window.RedVitaliaMetrics=api;
  function track(event,values={}){
    ...
    if(!activeGTM&&window.rvSendGA4)window.rvSendGA4(event,event_data);
  }
  function enable(){
  ...
  function setChoice(value){
    api.consent=value;api.hasResponded=true;choice=value;
    ...
    document.dispatchEvent(new Event('rv-consent-change'));
    panel.hidden=true;
  }
