...
const outputPath = path.join(root, "public", "data", "competitive-intelligence.json");
const scriptPath = path.join(root, "scripts", "build-competitive-intelligence.mjs");

const runBuilder = () => execFileSync(process.execPath, [scriptPath], { cwd: root, encoding: "utf8" });
const load = (file) => JSON.parse(readFileSync(file, "utf8"));

runBuilder();
const intelligence = load(outputPath);
const companies = load(path.join(root, "public", "data", "companies-index.json"));
...
test("publica el contrato canónico completo con cobertura material", () => {
  ...
  for (const key of ["scope", "methodology", "summary", "companyDna", "marketGaps", "playbooks", "offerMatrix", "patternLibrary", "hypothesisRanking"]) {
  ...
  assert.equal(intelligence.companyDna.length, intelligence.summary.eligibleCompanies);
  assert.equal(intelligence.offerMatrix.rows.length, intelligence.companyDna.length);
  assert.equal(new Set(intelligence.companyDna.map((row) => row.companyId)).size, intelligence.companyDna.length);
  for (const fingerprint of Object.values(intelligence.methodology.sourceFingerprints)) assert.match(fingerprint, /^[a-f0-9]{64}$/);
});
...
test("la cohorte no contiene fichas adyacentes, excluidas ni cuarentena conocida", () => {
  ...
  const allowedIds = new Set(intelligence.companyDna.map((row) => row.companyId));
  for (const row of intelligence.companyDna) {
    ...
    assert.ok(!forbiddenKnown.has(row.companyId), `contaminación conocida: ${row.companyId}`);
  }
  for (const pattern of intelligence.patternLibrary) {
    for (const evidence of pattern.evidence) assert.ok(allowedIds.has(evidence.companyId));
  }
  for (const hypothesis of intelligence.hypothesisRanking.items) {
    for (const evidence of hypothesis.evidence) assert.ok(allowedIds.has(evidence.companyId));
  }
});
...
test("deduplica identidades y mantiene denominadores ponderados por empresa", () => {
  ...
  assert.equal(
    intelligence.companyDna.reduce((sum, company) => sum + company.metrics.uniqueIdentities, 0),
    intelligence.summary.uniqueIdentities,
  );
  for (const pattern of intelligence.patternLibrary) {
    assert.ok(pattern.metrics.companies <= pattern.denominator.companies);
    assert.equal(pattern.metrics.adoptionPct, Math.round((pattern.metrics.companies / pattern.denominator.companies) * 1_000) / 10);
    ...
    assert.equal(new Set(pattern.evidence.map((item) => item.companyId)).size, pattern.evidence.length);
  }
});
...
test("huecos y ofertas publican denominadores válidos y porcentajes coherentes", () => {
  for (const vertical of intelligence.marketGaps.verticals) {
    ...
    assert.ok(vertical.denominator.landingCompanies <= vertical.denominator.companies);
    ...
    for (const gap of vertical.gaps) {
      ...
      assert.equal(gap.adoptionPct, Math.round((gap.observedCompanies / gap.denominatorCompanies) * 1_000) / 10);
  ...
  assert.equal(offer.summary.publicPriceObserved, offer.rows.filter((row) => row.pricing.status === "observed_attributed").length);
  assert.equal(offer.summary.guaranteeObserved, offer.rows.filter((row) => row.guarantee.status === "observed_attributed").length);
  for (const row of offer.rows) {
    assert.ok(row.evidence.length <= 1);
    if (row.pricing.status === "not_observed") assert.equal(row.pricing.excerpt, null);
    if (row.guarantee.status === "not_observed") assert.equal(row.guarantee.excerpt, null);
  }
});

test("ranking limita componentes a 0–100 y reproduce exactamente la fórmula", () => {
  const defaultWeights = { adoption: 25, activity: 20, longevity: 15, variants: 15, format: 10, landingCoherence: 15 };
  ...
  for (const item of intelligence.hypothesisRanking.items) {
    ...
    const available = Object.entries(item.components).filter(([, value]) => value !== null);
    for (const [, value] of available) assert.ok(value >= 0 && value <= 100);
    const weight = available.reduce((sum, [key]) => sum + defaultWeights[key], 0);
    const score = available.reduce((sum, [key, value]) => sum + value * defaultWeights[key], 0) / weight;
    assert.ok(Math.abs(item.score - Math.round(score * 10) / 10) <= 0.11, `${item.patternId}: ${item.score} vs ${score}`);
    assert.equal(item.availableWeight, weight);
    assert.deepEqual(item.weightsApplied, Object.fromEntries(available.map(([key]) => [key, defaultWeights[key]])));
  }
});

test("las evidencias publicitarias son trazables al corpus y están acotadas", () => {
  const evidenceIds = new Set(corpus.items.flatMap((ad) => [ad.externalId, ad.corpusKey]).filter(Boolean));
    ...
    for (const category of Object.values(company.signals)) {
      ...
      for (const value of category.values) {
        ...
        for (const evidence of value.evidence) {
          if (evidence.sourceType === "ad_copy") assert.ok(evidenceIds.has(evidence.adId), `ID no trazable: ${evidence.adId}`);
        }
      }
    }
  }
...
test("el lenguaje separa observación, inferencia y rendimiento", () => {
  assert.equal(intelligence.methodology.status, "descriptive_not_performance_validated");
  assert.match(intelligence.methodology.note, /no demuestran conversión, causalidad ni rendimiento/i);
  assert.match(intelligence.hypothesisRanking.disclaimer, /no contiene métricas de rendimiento/i);
  for (const pattern of intelligence.patternLibrary) {
    assert.equal(pattern.observationStatus, "observed_frequency");
    assert.equal(pattern.whenToUse.status, "inferred_recommendation");
    assert.match(pattern.saturationMeaning, /no equivale a fatiga ni rendimiento/i);
  }
  for (const item of intelligence.hypothesisRanking.items) {
    assert.doesNotMatch(item.claim, /ganador|demuestra|convierte mejor|funciona mejor/i);
    assert.match(item.interpretation, /necesita métricas propias/i);
  }
  for (const playbook of intelligence.playbooks) {
    assert.ok(Object.values(playbook.observedModules).every((module) => ["observed_frequency", "not_observed"].includes(module.status)));
    assert.ok(playbook.opportunityTests.every((item) => item.status === "inferred_hypothesis"));
  }
});
